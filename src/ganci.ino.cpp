# 1 "C:\\Users\\Moocchi\\AppData\\Local\\Temp\\tmpgz4fjh14"
#include <Arduino.h>
# 1 "C:/Users/Moocchi/Documents/PlatformIO/Projects/260224-190427-esp32-s3-devkitm-1/src/ganci.ino"
  #include <Update.h>
  #include <Arduino.h>
  #include <SPI.h>
  #include <Wire.h>
  ...
  const uint32_t DOUBLE_CLICK_GAP = 250;
  ...
  bool decodeQOI(File& f, uint16_t* outBuf) {
      ...
      void* decodedPixels = qoi_decode(qoiData, fSize, &desc, 4);
      free(qoiData);
  ...
  void showBinFile(String filename) {
      ...
      uint16_t* imgBuf = (uint16_t*)ps_malloc(115200);
      if (!imgBuf) { Serial.println("MEDIA: OOM!"); f.close(); return; }

      if (isQoi) {
          decodeQOI(f, imgBuf);
      } else if (fSize >= 230400) {
              ...
              while (entry) {
                  ...
                  if (elow.startsWith("frame") && (elow.endsWith(".bin") || elow.endsWith(".qoi"))) {
                      ...
                      if (firstFrameIdx == -1 || idx < firstFrameIdx) {
                          firstFrameIdx = idx;
                          seqExt = ename.substring(ename.lastIndexOf('.'));
                      }
                  }
...
void setup();
#line 699 "C:/Users/Moocchi/Documents/PlatformIO/Projects/260224-190427-esp32-s3-devkitm-1/src/ganci.ino"
float easeOutCubic(float t) {
...
void runRealtimeClock() {
    ...
    const float W = 5.0f;
    const float r_in = R - W/2.0f;
    const float r_out = R + W/2.0f;
    ...
    bool needResync = !ntpEverSynced || (now_entry - lastNtpSyncMs > 3600000UL);
    ...
    while (true) {
        ...
        int currentSec = hasTime ? t.tm_sec : -2;
...
# 1023 "C:/Users/Moocchi/Documents/PlatformIO/Projects/260224-190427-esp32-s3-devkitm-1/src/ganci.ino"
...
void setup() {
  ...
  void loop() {
# 1241 "C:/Users/Moocchi/Documents/PlatformIO/Projects/260224-190427-esp32-s3-devkitm-1/src/ganci.ino"
